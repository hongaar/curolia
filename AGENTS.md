...
- **Baselines:** each run writes **`tests/.metrics/metrics.json`** (gitignored); compare with **`npm run compare-baseline -w @curolia/e2e`** against **`tests/baselines/main.json`** (counters: any increase; timings: +25%). PRs fail on regressions (`E2E_ENFORCE_BASELINE=1`); **`main`** reports warnings and does not block CD. Do **not** auto-commit the baseline — update it with **Actions → Update E2E baseline** (or `--refresh-baseline` locally). See **`tests/e2e/README.md`**.
