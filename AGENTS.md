...
- **Baselines:** each run writes **`tests/.metrics/metrics.json`** (gitignored); compare with **`npm run compare-baseline -w @curolia/e2e`** against **`tests/baselines/main.json`** (counters fail when they rise by more than 2; timings fail beyond +25%). CI reports the delta on every run, enforces it on pull requests, and refreshes the baseline on pushes to `main`. See **`tests/e2e/README.md`** for full docs.
