-- Readable URL segments: maps and tags get stable slug columns (globally unique
-- maps, per-map unique tags).
...
-- Globally unique map slug; p_map_id excludes the current row (NULL on insert OK).
...
revoke all on function public.map_claim_slug(uuid, text) from public;
grant execute on function public.map_claim_slug(uuid, text)
...
-- -----------------------------------------------------------------------------
-- Maps: backfill slug, defaults, triggers
-- -----------------------------------------------------------------------------

update public.maps j
set slug = public.map_claim_slug(j.id, j.name)
...
begin
  if tg_op = 'INSERT' then
    if new.slug is null or trim(new.slug) = '' then
      new.slug := public.map_claim_slug(new.id, new.name);
    else
      new.slug := public.map_claim_slug(new.id, new.slug);
    end if;
...
drop trigger if exists maps_set_slug_before_ins on public.maps;

create trigger maps_set_slug_before_ins
...
create trigger maps_set_slug_before_upd
  before update of slug on public.maps
  for each row execute function public.maps_set_slug();
...
set slug = public.tag_claim_slug(t.map_id, t.id, t.name)
where t.slug is null or trim(t.slug) = '';

alter table public.tags
  alter column slug set not null;

drop index if exists tags_map_slug_key;

create unique index tags_map_slug_key
  on public.tags (map_id, slug);
...
begin
  if tg_op = 'INSERT' then
    if new.slug is null or trim(new.slug) = '' then
      new.slug := public.tag_claim_slug(new.map_id, new.id, new.name);
    else
      new.slug := public.tag_claim_slug(new.map_id, new.id, new.slug);
    end if;
  elsif tg_op = 'UPDATE' then
