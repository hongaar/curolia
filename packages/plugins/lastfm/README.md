...
3. Open a **pin** with a **start date** (and optional end date). The pin detail page syncs when dates are set; changing dates invalidates the sync query key (**`lastfmPinSyncQueryKey`** in **`src/query-keys.ts`**).

## Behaviour notes

- Uses **`user.getRecentTracks`** with **`from`** / **`to`** (Unix **seconds**, UTC), aligned with pin bounds (**UTC calendar days** from `YYYY-MM-DD`).
- Long windows are capped by **`LASTFM_MAX_PAGES`** × **`LASTFM_PAGE_LIMIT`** (see **`src/constants.ts`**).
- Payload shape for `plugin_entity_data.data` is **`LastfmPinPayload`** in **`src/lastfm-pin-data.ts`** (`schemaVersion: 1`).
