...
import {
  COMMONS_NEARBY_CANDIDATES_LIMIT,
  commonsExternalRef,
  parseCommonsCandidate,
  searchCommonsNearby,
  type CommonsNearbyCandidate,
} from "./lib/commons-api.ts";

const PLUGIN_ID = "commons";
...
async function existingCommonsFileTitles(
  admin: ReturnType<typeof createClient>,
  pinId: string,
): Promise<Set<string>> {
  const { data, error } = await admin
  ...
  for (const row of data) {
    const ref = row.external_ref as { fileTitle?: unknown } | null;
    if (typeof ref?.fileTitle === "string") titles.add(ref.fileTitle);
  }
  return titles;
}
...
Deno.serve(async (req: Request) => {
  ...
  if (body.action === "list_nearby") {
    try {
      const existing = await existingCommonsFileTitles(admin, pinId);
      const candidates = (
        await searchCommonsNearby(lat, lng, COMMONS_NEARBY_CANDIDATES_LIMIT)
      ).filter((c) => !existing.has(c.fileTitle));
      ...
      console.error("commons list_nearby failed", e);
      return new Response(JSON.stringify({ error: "commons_search_failed" }), {
        status: 502,
        headers: { ...cors(), "Content-Type": "application/json" },
      });
    }
  ...
  if (body.action === "attach") {
    ...
    const parsed: CommonsNearbyCandidate[] = [];
    for (const raw of rawCandidates) {
      const candidate = parseCommonsCandidate(raw);
    ...
    for (const candidate of parsed) {
      if (existing.has(candidate.fileTitle)) {
        skippedAlreadyOnPin.push(candidate.fileTitle);
        continue;
      }

      sort += 1;
      const external_ref = commonsExternalRef(candidate);
      ...
      existing.add(candidate.fileTitle);
    }
